...
def no_apples_for_blair_sub
  ...
  FROM 
    cats
  WHERE
    cats.name != 'Blair' AND cats.id IN (
      SELECT 
        cat_id
      FROM
        cattoys
...
def price_like_shiny_mouse
  ...
  JOIN
    toys
  ON
    shiny_toys.price = toys.price
  WHERE
    shiny_toys.name = 'Shiny Mouse' AND toys.name != 'Shiny Mouse'
  ORDER BY toys.name;  
  SQL
end
...
def just_like_orange
  ...
  FROM
    cats AS orange_breed
  JOIN
    cats 
  ON
    orange_breed.breed = cats.breed
  WHERE
    orange_breed.name = 'Orange' AND cats.name != 'Orange'
  ORDER BY cats.name;
  SQL
end
...
def just_like_orange_sub
  ...
  FROM
    cats
  WHERE
    cats.name != 'Orange' AND cats.breed = (
      SELECT
        cats.breed
      FROM
        cats
...
def toys_that_jet_owns
  ...
  FROM
    cats AS jet
  JOIN  
    cattoys AS jets_toys
  ON
    jets_toys.cat_id = jet.id
  JOIN
    toys
  ON
    jets_toys.toy_id = toys.id
  JOIN
    cattoys
  ON  
  ...
  ON
    cats.id = cattoys.cat_id
  WHERE
    jet.name = 'Jet'AND cats.name != 'Jet' AND toys.id IN (jets_toys.toy_id)
  ORDER BY cats.name;
...
def toys_that_jet_owns_sub
  ...
  ON
    toys.id = cattoys.toy_id
  WHERE
   cats.name != 'Jet' AND cattoys.toy_id IN (
  SELECT
    toys.id
  FROM
    toys
